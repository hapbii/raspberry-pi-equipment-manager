...
def main() -> int:
    ...
    return run_memory_check(app, args)


def run_memory_check(app, args) -> int:
    try:
        exit_code = _run_memory_check(app, args)
    except KeyboardInterrupt:
        print("\n사용자가 검사를 중지했습니다.")
        exit_code = 130
    finally:
        # Cover preflight, warmup, output errors and interrupted measurements.
        app.extensions["shutdown_services"]()
    gc.collect()
    print(f"서비스 종료 후 RSS: {current_rss_mb()} MB")
    return exit_code


def _run_memory_check(app, args) -> int:
    ...
    print(f"앞·뒤 구간 중앙값 증가량: {trend} MB")
    if successes == 0:
        print("실패: 인식 성공이 없어 정상 추론의 메모리 사용을 검증하지 못했습니다.")
        return 1
