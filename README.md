...
카메라 앞에 기자재를 놓으면 Raspberry Pi가 기자재의 **종류**를 인식합니다. 학생은 학번과 대여 사유를 입력하고, 인식 결과의 **“OOO 기자재입니다.”** 안내를 확인해 대여합니다. 기자재 선택과 수량 입력 없이 화면에서는 한 번에 1개씩 처리하며, 반납에는 사유 입력이 필요하지 않습니다. 다른 PC나 스마트폰에서는 현재 남은 수량을 웹사이트로 확인할 수 있습니다. 반납 예정일은 관리자가 기자재마다 설정한 대여 기간으로 자동 계산됩니다. 반납 기한이 지나면 해당 학번은 연체 기자재를 모두 반납할 때까지 새로 대여할 수 없습니다.
...
- 개발자 관리자 아이디와 비밀번호
- 선생님 관리자 아이디와 비밀번호
- 최종 대여·반납 처리에 사용할 스테이션 PIN

두 계정의 아이디와 비밀번호를 각각 안전한 곳에 기록합니다. 개발자 계정은 모든 관리 기능과 시스템 진단 화면을 사용할 수 있고, 선생님 계정은 기자재·미반납·거래 관리 기능만 사용할 수 있습니다. 대여·반납 화면에는 누구나 들어갈 수 있지만, 개발자 이외에는 마지막 `이 기자재 대여하기` 또는 `이 기자재 반납하기` 단계에서 스테이션 PIN을 확인합니다.

| 계정 | 기자재·거래 관리 | 대여·반납 | 시스템 진단 |
|---|---:|---:|---:|
| 선생님 관리자 | 가능 | PIN 확인 후 가능 | 접근 불가 |
| 개발자 관리자 | 가능 | PIN 없이 가능 | 가능 |

두 계정 모두 기자재 종류를 추가·수정·제거할 수 있습니다. 최근 거래를 영구 삭제하는 기능만 개발자 계정으로 제한됩니다.

배포용에서는 `.env`의 `STATION_AUTH_REQUIRED=true`가 필수입니다. 학생·선생님은 학번·구분·대여 사유 입력과 객체 인식까지 PIN 없이 진행하고, 마지막 저장 버튼을 누를 때 PIN 입력창이 나타납니다. 올바른 `STATION_PIN`을 입력해야 대여·반납이 저장됩니다. **개발자로 로그인한 경우만 PIN 확인을 생략**하며 현황·대여·반납·관리자·시스템 화면과 모든 관리 기능에 접근할 수 있습니다. 개발자도 기자재 수량·대여 기한 등 거래 규칙은 그대로 적용받습니다. 공용 단말에서 개발자 작업이 끝나면 반드시 로그아웃하세요.
...
관리자 화면은 `http://<PI_IP>:8080/admin/login`에서 접속합니다. 선생님은 `.env`의 `TEACHER_USERNAME`과 `TEACHER_PASSWORD`, 개발자는 `DEVELOPER_USERNAME`과 `DEVELOPER_PASSWORD`를 입력합니다. 개발자 계정으로 로그인하면 상단에 `시스템` 메뉴가 추가됩니다. 대여·반납 화면은 `http://<PI_IP>:8080/scan`으로 바로 접속하며, 기본 설정에서는 최종 거래 저장 시 스테이션 PIN을 입력합니다.
...
- `현황`: 누구나 현재 수량 확인
- `대여·반납`: 화면과 카메라 인식은 바로 사용하고 최종 거래 저장 시 PIN 확인
...
8. `이 기자재 대여하기`를 누릅니다. 화면에서는 한 번에 1개씩 처리합니다.
9. 팝업창에 스테이션 PIN을 입력하고 `확인 후 처리`를 누릅니다.
10. 현황 화면에서 사용 가능 수량이 줄었는지 확인합니다.

## 학생이 반납할 때

같은 순서에서 `반납`을 선택합니다. 사유 입력란은 숨겨지고 학번만 입력하면 됩니다. 카메라 인식 결과를 확인한 뒤 `이 기자재 반납하기`를 누르고 PIN을 입력합니다 (개발자만 면제). 해당 학번이 빌린 수량보다 많이 반납할 수는 없습니다.
...
PIN 중복 제출과 요청 시간 초과·페이지 이동 시 정리도 확인할 수 있습니다. Node.js가 설치된 PC에서 실행합니다. Raspberry Pi 서버 실행을 위해 Node.js를 설치할 필요는 없습니다.
...
- [ ] 최종 대여·반납 처리에 사용할 스테이션 PIN을 기록했다.
- [ ] DB를 백업했다.
