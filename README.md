...
관리자 아이디와 비밀번호는 종이나 비밀번호 관리 앱에 기록합니다. 기본 설정에서는 대여·반납 화면에 PIN 없이 바로 들어갈 수 있습니다.
...
이전 버전에서 이미 `.env`를 만든 경우에는 파일이 자동으로 덮어써지지 않습니다. 아래 명령으로 파일을 열어 새 설정 두 줄을 직접 추가합니다.
...
`ADMIN_USERNAME`은 원하는 관리자 아이디로 바꿔도 됩니다. `false`이면 대여·반납 화면에 바로 들어가고, `true`이면 기존 `STATION_PIN`을 입력해야 합니다. 저장한 뒤에는 실행 중인 서버나 서비스를 다시 시작합니다.
...
관리자 화면은 `http://<PI_IP>:8080/admin/login`에서 접속합니다. `.env`에 저장된 `ADMIN_USERNAME`과 `ADMIN_PASSWORD`를 모두 입력해야 합니다. 대여·반납 화면은 기본 설정에서 `http://<PI_IP>:8080/scan`으로 바로 접속됩니다.
...
- `관리자`: 관리자 아이디와 비밀번호로 로그인해 종류·수량·기록 관리
