...
아래 설치 명령은 **부팅 자동 실행 + 개발자 전용 라파 종료 버튼**을 함께 준비합니다. 한 번 설치하면 이후에는 Pi에 전원을 연결해 부팅하고 네트워크 연결이 완료될 때까지 기다리면 됩니다. 터미널 로그인, 폴더 이동, 가상환경 활성화 명령을 매번 입력하지 않습니다. 부팅 직후 즉시 접속되는 것은 아니며 네트워크 연결에 시간이 걸릴 수 있습니다. 공유기에서 Pi의 IP를 예약해 두면 주소가 바뀌는 불편을 줄일 수 있습니다.
...
`--enable-poweroff`는 종료 버튼의 운영체제 권한까지 설치합니다. 종료 기능 없이 자동 실행만 원하면 이 옵션을 빼세요. 이후 설치기를 다시 실행할 때도 종료 기능을 유지하려면 이 옵션을 붙여야 합니다. 옵션을 빼고 재설치하면 웹 종료 기능과 설치기가 만든 종료 권한을 해제합니다. **설치 과정에서 Pi를 종료하거나 종료 타이머를 시작하지는 않습니다.**
...
학생·선생님에게는 종료 버튼이 표시되지 않고, 종료 주소로 직접 요청해도 서버에서 거부합니다. 개발자도 로그인 세션·CSRF 토큰·비밀번호 재확인을 모두 통과해야 합니다. 잘못된 비밀번호를 포함한 종료 요청은 5분에 5회로 제한합니다. 종료 직전 새 거래를 자동 차단하는 기능은 아니므로 **사용 중인 사람이 없는지 확인한 뒤** 실행하세요.
...
**권한 설계:** 웹서버는 일반 사용자로 실행하고 `NoNewPrivileges=true`를 유지합니다. root 소유 종료 타이머 하나의 `start`만 해당 Linux 사용자에게 polkit으로 허용합니다. 임의 명령·서비스 제어·sudo 전체 권한은 추가하지 않습니다. 이 OS 권한은 웹 계정이 아니라 **서버 실행 Linux 사용자** 기준이므로 같은 Linux 사용자로 실행되는 다른 프로그램도 그 타이머를 요청할 수 있습니다. `.env`, 개발자 계정, Pi 사용자 계정을 안전하게 관리하고 실제 학교 배포에서는 HTTPS도 구성하세요. 구현 근거: [systemd 권한 검사 구현](https://github.com/systemd/systemd/blob/main/src/core/dbus-util.c), [systemd 타이머 설명](https://github.com/systemd/systemd/blob/main/man/systemd.timer.xml).
