...
두 계정의 아이디와 비밀번호를 각각 안전한 곳에 기록합니다. 개발자 계정은 모든 관리 기능과 시스템 진단 화면을 사용할 수 있고, 선생님 계정은 기자재·미반납·거래 관리 기능만 사용할 수 있습니다. 대여·반납 화면에는 누구나 들어갈 수 있지만, 기본 설정에서는 마지막 `이 결과로 처리` 단계에서 스테이션 PIN을 확인합니다.
...
배포용에서는 `.env`의 `STATION_AUTH_REQUIRED=true`가 필수입니다. 학생·선생님은 학번·구분·수량 입력과 객체 인식까지 PIN 없이 진행하고, 마지막 저장 버튼을 누를 때 PIN 입력창이 나타납니다. 올바른 `STATION_PIN`을 입력해야 대여·반납이 저장됩니다. **개발자로 로그인한 경우만 PIN 확인을 생략**하며 현황·대여·반납·관리자·시스템 화면과 모든 관리 기능에 접근할 수 있습니다. 개발자도 기자재 수량·대여 기한 등 거래 규칙은 그대로 적용받습니다. 공용 단말에서 개발자 작업이 끝나면 반드시 로그아웃하세요.
