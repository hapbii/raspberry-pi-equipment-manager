...
`.env`의 `STATION_AUTH_REQUIRED=true`이면 학번·구분·수량 입력과 객체 인식까지는 PIN 없이 진행되고, 마지막 저장 버튼을 누를 때 PIN 입력창이 나타납니다. 올바른 `STATION_PIN`을 입력해야 거래가 저장됩니다. PIN 확인을 사용하지 않으려면 `STATION_AUTH_REQUIRED=false`로 변경할 수 있습니다. 개발자 계정으로 로그인한 상태에서는 PIN 확인을 생략합니다.
