...
test('stop policy grants only the selected user starting either fixed timer', () => {
  let rule;
  ...
  for (const timer of ['equipment-manager-poweroff.timer', 'equipment-manager-stop.timer']) {
    ...
    for (const unit of ['ssh.service', 'equipment-manager-poweroff.service', 'equipment-manager-stop.service', 'equipment-manager.service', 'poweroff.target', undefined]) {
